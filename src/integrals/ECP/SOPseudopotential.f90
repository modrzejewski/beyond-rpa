...
contains
      ...
      subroutine sop_V_2(vx, vy, vz, AOBasis, System)
            !
            ...
            associate ( &
                  ...
                  call sop_allocate_scratch(tcc, tab, tac, slma, slmb, xyzwork)
                  ...
                  do shellab = 1, shellab_max
                        ...
                        do cc = 1, ECP_NATOM
                              ...
                              if ((a .ne. c) .and. (b .ne. c)) then
                                    call sop_ecpab(gabx, gaby, gabz, a, b, c, phia, phib, slma, slmb, tab, System)
                              else if ((a .ne. c) .and. (b .eq. c)) then
                                    call sop_ecpac(gabx, gaby, gabz, a, c, phia, phib, slma, tac, xyzwork, System)
                              else if ((a .eq. c) .and. (b .ne. c)) then
                                    call sop_ecpbc(gabx, gaby, gabz, b, c, phia, phib, slma, tac, xyzwork, System)
                              else
                                    !
                                    ! a == c .and. b == c
                                    !
                                    call sop_ecpcc(gabx, gaby, gabz, c, phia, phib, tcc, xyzwork, System)
                              end if
                        end do
                        !
                        ...
                        !
                        call pp_vmat_update(vx, gabx, shella, shellb, phia, phib, AOBasis)
                        call pp_vmat_update(vy, gaby, shella, shellb, phia, phib, AOBasis)
                        call pp_vmat_update(vz, gabz, shella, shellb, phia, phib, AOBasis)
                  end do
                  !$omp end do nowait
                  deallocate(tcc)
                  deallocate(tab)
      ...
      end subroutine sop_V_2


      subroutine sop_allocate_scratch(tcc, tab, tac, slma, slmb, xyzwork)
      ...
      end subroutine sop_allocate_scratch


      subroutine sop_ecpab(gabx, gaby, gabz, a, b, c, phia, phib, slma, slmb, tab, System)
            !
            ...
            do j = 1, nfuncb
                  ...
                  do i = 1, nfunca
                        ...
                        lay = ECP_MM(i, la)
                        laz = ECP_NN(i, la)
                        call sop_gamab(gamab, lax, lay, laz, lbx, lby, lbz, ax, ay, az, bx, by, bz, &
      ...
      end subroutine sop_ecpab


      subroutine sop_ecpac(gabx, gaby, gabz, a, c, phia, phib, slma, tac, xyzwork, System)
            !
            ! 1. Flores-Moreno, R., Alvarez-Mendez, R.J., Vela, A., Koster, A.M. J. Comput. Chem. 27, 1009 (2006);
            !    doi: 10.1002/jcc.20410
            !
            ...
            !
            call sop_preptac(tac, phia, phib, lena, ecpcenter, lmax)
            v = 1
            !
            ...
            do j = 1, nfuncb
                  ...
                  do i = 1, nfunca
                        ...
                        lay = ECP_MM(i, la)
                        laz = ECP_NN(i, la)
                        call sop_gamac(gamac, lax, lay, laz, lbx, lby, lbz, ax, ay, az, SLMA, ecpcenter, tac, xyzwork)
      ...
      end subroutine sop_ecpac


      subroutine sop_ecpbc(gabx, gaby, gabz, b, c, phia, phib, slmb, tbc, xyzwork, System)
            real(F64), dimension(:), intent(inout)        :: gabx
            real(F64), dimension(:), intent(inout)        :: gaby
            real(F64), dimension(:), intent(inout)        :: gabz
            integer, intent(in)                           :: b
            ...
            ! orbitals that differ by in an angular function.
            !
            call sop_preptac(tbc, phib, phia, lenb, ecpcenter, lmax)
            ...
            do j = 1, nfuncb
                  ...
                  do i = 1, nfunca
                        ...
                        lay = ECP_MM(i, la)
                        laz = ECP_NN(i, la)
                        call sop_gamac(gamac, lbx, lby, lbz, lax, lay, laz, bx, by, bz, SLMB, ecpcenter, tbc, xyzwork)
                        !
      ...
      end subroutine sop_ecpbc
      

      subroutine sop_preptac(tac, phia, phib, lena, ecpcenter, lmax)
            real(F64), dimension(0:, 1:, 0:), intent(out) :: tac
            type(tgtodef), intent(in)                     :: phia
            type(tgtodef), intent(in)                     :: phib 
      ...
      end subroutine sop_preptac
      
      
      subroutine sop_ecpcc(gabx, gaby, gabz, c, phia, phib, tcc, xyzwork, System)
            !
            ! 1. Flores-Moreno, R., Alvarez-Mendez, R.J., Vela, A., Koster, A.M. J. Comput. Chem. 27, 1009 (2006);
            !    doi: 10.1002/jcc.20410
            !
            ...
            call sop_preptcc(tcc, phia, phib, ecpcenter, lmax)
            ...
            do j = 1, nfuncb
                  ...
                  do i = 1, nfunca
                        ...
                        lay = ECP_MM(i, la)
                        laz = ECP_NN(i, la)
                        call sop_gamcc(gamcc, lax, lay, laz, lbx, lby, lbz, ecpcenter, tcc, xyzwork)
                        !
      ...
      end subroutine sop_ecpcc


      subroutine sop_preptcc(tcc, phia, phib, ecpcenter, lmax)
            real(F64), dimension(:), intent(out) :: tcc
            type(tgtodef), intent(in) :: phia
            type(tgtodef), intent(in) :: phib
            integer, intent(in) :: ecpcenter
      ...
      end subroutine sop_preptcc


      pure function sop_ccint(tp)
            real(F64) :: sop_ccint
      ...
      end function sop_ccint

      
      subroutine sop_gamab(gamab, lax, lay, laz, lbx, lby, lbz, ax, ay, az, &
                                    ...
                                    do bety = 0, lby
                                          ...
                                          do betz = 0, lbz
                                                ...
                                                do l = 1, ECP_LMAX(ecpcenter)
                                                      call sop_gamabl(gamabl, l, alpx, alpy, alpz, betx, &
      ...
      end subroutine sop_gamab


      subroutine sop_gamcc(gamcc, lax, lay, laz, lbx, lby, lbz, ecpcenter, tcc, xyzwork)
            ...
            !
            do l = 1, ECP_LMAX(ecpcenter)
                  call sop_gamccl(gamccl, l, lax, lay, laz, lbx, lby, lbz, tcc, xyzwork)
                  gamcc = gamcc + gamccl
            end do
      end subroutine sop_gamcc


      subroutine sop_gamac(gamac, lax, lay, laz, lbx, lby, lbz, ax, ay, az, &
            ...
            do alpx = 0, lax
                  ...
                  do alpy = 0, lay
                        ...
                        do alpz = 0, laz
                              ...
                              !
                              do l = 1, ECP_LMAX(ecpcenter)
                                    call sop_gamacl(gamacl, l, alpx, alpy, alpz, &
      ...
      end subroutine sop_gamac
      

      subroutine sop_gamabl(gamabl, l, alpx, alpy, alpz, betx, bety, betz, wslma, wslmb, tab)
            !
      ...
      end subroutine sop_gamabl


      subroutine sop_gamacl(gamacl, l, alpx, alpy, alpz, lbx, lby, lbz, wslma, tac, xyzwork)
            !
            ...
            !
            ! See the comments for SOP_GAMABL for more details.
            !
      ...
      end subroutine sop_gamacl


      subroutine sop_gamccl(gamccl, l, lax, lay, laz, lbx, lby, lbz, tcc, xyzwork)
            !
            ! Integrate the spin-orbit part of the pseudopotential. This variant of the subroutine
            ! works for A = B = C, where A, B are the centers of PhiA and PhiB;
            ! the pseudopotential is centered on C.
      ...
      end subroutine sop_gamccl


      pure function sop_ulpot(r, l, ecpcenter, k0)
            ! ----------------------------------------------------
            ...
            do k = k0, k0 + ngauss - 1
                  ...
                  nkl = ECP_NKL(k)
                  e = exp(-alpha * rn(2))
                  sop_ulpot = sop_ulpot + c * rn(nkl) * e
            end do
      end function sop_ulpot


      pure function sop_tintab(r, tp)
            real(F64)              :: sop_tintab
            real(F64), intent(in)  :: r
            type(TINT_PARAMS), intent(in) :: tp
            ...
            associate (phia => tp%phia, phib => tp%phib, &
                  ...
                  ul = sop_ulpot(r, l, ecpcenter, k0)
                  fa = pp_falambda(r, lambdaa, phia, lena, alpha)
                  fb = pp_falambda(r, lambdab, phib, lenb, beta)
                  sop_tintab = fa * fb * ul
            end associate
      end function sop_tintab


      pure function sop_tintac(r, tp)
            real(F64)              :: sop_tintac
            real(F64), intent(in)  :: r
            type(TINT_PARAMS), intent(in) :: tp
            ...
            associate (phia => tp%phia, phib => tp%phib, &
                  ...
                  sop_tintac = fa * phibval * ul
            end associate
      end function sop_tintac
end module SOPseudopotential
