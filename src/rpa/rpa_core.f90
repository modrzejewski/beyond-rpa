...
contains
      ...
      subroutine rpa_Wabcd(W, RhoOcc, RhoVirt, LaplaceW, Na, Nb, Nc, Nd, &
            ...
            real(F64), dimension(NLaplace, NAO, NAO), intent(in) :: RhoOcc
            real(F64), dimension(NLaplace, NAO, NAO), intent(in) :: RhoVirt
                              ...
                              do p = 1, Na
                                    ...
                                    do k = 1, NLaplace
                                          Tabcd(p+(q-1)*Na, r+(s-1)*Nc) = Tabcd(p+(q-1)*Na, r+(s-1)*Nc) + &
                                                LaplaceW(k) * ( &
                                                RhoOcc(k, pp, rr) * RhoVirt(k, qq, ss) + &
                                                RhoOcc(k, qq, rr) * RhoVirt(k, pp, ss) + &
                                                RhoOcc(k, pp, ss) * RhoVirt(k, qq, rr) + &
                                                RhoOcc(k, qq, ss) * RhoVirt(k, pp, rr))
                                    end do
                              end do
                        end do
                  end do
            ...
            W = Tabcd
      end subroutine rpa_Wabcd


      subroutine rpa_Wabcc(W, RhoOcc, RhoVirt, LaplaceW, Na, Nb, Nc, &
            ...
            W = Tabcd
      end subroutine rpa_Wabcc


      subroutine rpa_Waacd(W, RhoOcc, RhoVirt, LaplaceW, Na, Nc, Nd, &
            ...
            W = Tabcd
      end subroutine rpa_Waacd


      subroutine rpa_Waacc(W, RhoOcc, RhoVirt, LaplaceW, Na, Nc, &
            ...
            W = Tabcd
      end subroutine rpa_Waacc


      subroutine rpa_W(W, NBra, NKet, RhoOcc, RhoVirt, ShellPairs, ShellPairLoc, &
            ShellPairDim, ShellLoc, ShellParamsIdx, NAngFunc, LaplaceW, NAO, &
            NLaplace, BraBounds, KetBounds, IgnoreParity)
            !
            ! Compute a single block of the matrix W(pq,rs), where pq and rs belong
            ! to the subsets defined by BraBounds and KetBounds.
            !
            ! The bra-ket shell pair indices are restricted so that only a half of
            ! the symmetric matrix W is computed. The rest of the matrix W will be
            ! accounted for at a later stage by computing GRWRG <- GRWRG + GRWRG**T.
            ! The diagonal shell blocks of W are scaled by 1/2.
            !
            ! The simplest algorithm for selecting (ab, cd) employs the condition
            ! ab>=cd, but that makes the parallelized loops over cd inefficient.
            ! The amount of work done for different cd's can vary widely,
            ! which is determiental to the performance of a parallel program
            ! if the loop over cd's is the main parallel loop.
            !
            ! To take advantage of the symmetry W(pq,rs)=W(rs,pq) in an efficient way,
            ! a more complicated algorithm is used which checks the parity of ab and cd:
            !
            ! if (modulo(ab, 2) == modulo(cd, 2) .and. ab >= cd) accept
            ! if (modulo(ab, 2) /= modulo(cd, 2) .and. ab < cd) accept
            ! discard otherwise
            !
            ! Example
            ! --------
            ! accepted:  (1, 3) (4, 1) (5, 2) (2, 4)
            ! discarded: (3, 1) (1, 4) (2, 5) (4, 2)
            !
            ...
            if (IgnoreParity) then
                  ...
                  do ShCD = KetBounds(1), KetBounds(2)
                        do ShAB = BraBounds(1), BraBounds(2)
                              ...
                              if (ShC /= ShD) then
                                    if (ShA /= ShB) then
                                          call rpa_Wabcd(W(pq0:pq1, rs0:rs1), RhoOcc, RhoVirt, &
                                                LaplaceW, Na, Nb, Nc, Nd, LocA, LocB, LocC, LocD, Nab, Ncd, &
                                                NAO, NLaplace)
                                    else
                                          call rpa_Waacd(W(pq0:pq1, rs0:rs1), RhoOcc, RhoVirt, &
                                                LaplaceW, Na, Nc, Nd, LocA, LocC, LocD, Nab, Ncd, &
                                                NAO, NLaplace)
                                    end if
                              else
                                    if (ShA /= ShB) then
                                          call rpa_Wabcc(W(pq0:pq1, rs0:rs1), RhoOcc, RhoVirt, &
                                                LaplaceW, Na, Nb, Nc, LocA, LocB, LocC, Nab, Ncd, &
                                                NAO, NLaplace)
                                    else
                                          call rpa_Waacc(W(pq0:pq1, rs0:rs1), RhoOcc, RhoVirt, &
      ...
      end subroutine rpa_W


      subroutine rpa_WRG(WRG, RG, ldRG, W, NBra, NKet, RhoOcc, RhoVirt, &
            ...
            call rpa_W(W, NBra, NKet, RhoOcc, RhoVirt, ShellPairs, ShellPairLoc, &
                  ShellPairDim, ShellLoc, ShellParamsIdx, NAngFunc, LaplaceW, NAO, &
                  NLaplace, BraBounds, KetBounds, IgnoreParity)
      ...
      end subroutine rpa_WRG


      subroutine rpa_GPiUG_BraLoop(GPiUG, WRG, W, RGBra, RGKet, RhoOcc, RhoVirt, ShellPairs, ShellPairLoc, &
            ShellPairDim, ShellLoc, LaplaceW, ShellParamsIdx, NAngFunc, NAO, NVecsPiU, &
            ...
            do Bx = 1, NSubsets(1)
                  ...
                  if (NBra > 0 .and. NKet > 0 .and. &
                        ...
                        !
                        call rpa_WRG(WRG, RGBra(:, :, Bx), ldRGBra, W(1:NBra*NKet), NBra, NKet, RhoOcc, RhoVirt, &
                              ShellPairs, ShellPairLoc, ShellPairDim, ShellLoc, ShellParamsIdx, NAngFunc, LaplaceW, &
                              NAO, NLaplace, NVecsPiU, SubsetBounds(:, Bra), &
            ...
            !
            call clock_start(timer)
            call real_abT_x(GPiUG, NVecsPiU, RGKet, ldRGKet, WRG, NVecsPiU, &
                  NVecsPiU, NVecsPiU, NKet, alpha=ONE, beta=ONE)
            time_GRWRG = time_GRWRG + clock_readwall(timer)
      end subroutine rpa_GPiUG_BraLoop


      subroutine rpa_GPiUG(GPiUG, WRG, W, RGBra, RGKet, RhoOcc, RhoVirt, ShellPairs, ShellPairLoc, &
            ShellPairDim, ShellLoc, LaplaceW, ShellParamsIdx, NAngFunc, NAO, NVecsPiU, &
