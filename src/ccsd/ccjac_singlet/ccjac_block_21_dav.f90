...
implicit none
!
! File generated automatically on 2013-07-13 00:47:06 UTC.
!
contains
subroutine ccjac_21_dav(sigup_diag, sigup_nondiag, t2, nocc0, nocc1, nvirt0, nvirt1, &
...
iket = ketoffset + ck
jac_ibra_iket = eom_ccsd_21_trans_aiajck(t2, nocc, nactive, a, i, j, c, k)
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac_ibra_iket = eom_ccsd_21_trans_aibjak(t2, nocc, nactive, a, i, b, j, k)
call sigup_nondiag(jac_ibra_iket, ibra, iket)
 ! jac_temp(ibra, iket) = jac_ibra_iket 
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac_ibra_iket = eom_ccsd_21_trans_aibjbk(t2, nocc, nactive, a, i, b, j, k)
call sigup_nondiag(jac_ibra_iket, ibra, iket)
 ! jac_temp(ibra, iket) = jac_ibra_iket 
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac_ibra_iket = eom_ccsd_21_trans_aibick(t2, nocc, nactive, a, i, b, c, k)
call sigup_nondiag(jac_ibra_iket, ibra, iket)
 ! jac_temp(ibra, iket) = jac_ibra_iket 
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac_ibra_iket = eom_ccsd_21_trans_aibjci(t2, nocc, nactive, a, i, b, j, c)
call sigup_nondiag(jac_ibra_iket, ibra, iket)
 ! jac_temp(ibra, iket) = jac_ibra_iket 
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac_ibra_iket = eom_ccsd_21_trans_aibjcj(t2, nocc, nactive, a, i, b, j, c)
call sigup_nondiag(jac_ibra_iket, ibra, iket)
 ! jac_temp(ibra, iket) = jac_ibra_iket 
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac_ibra_iket = eom_ccsd_21_trans_aiajak(t2, nocc, nactive, a, i, j, k)
call sigup_nondiag(jac_ibra_iket, ibra, iket)
 ! jac_temp(ibra, iket) = jac_ibra_iket 
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac_ibra_iket = eom_ccsd_21_trans_aiaick(t2, nocc, nactive, a, i, c, k)
call sigup_nondiag(jac_ibra_iket, ibra, iket)
 ! jac_temp(ibra, iket) = jac_ibra_iket 
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac_ibra_iket = eom_ccsd_21_trans_aiajci(t2, nocc, nactive, a, i, j, c)
call sigup_nondiag(jac_ibra_iket, ibra, iket)
 ! jac_temp(ibra, iket) = jac_ibra_iket 
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac_ibra_iket = eom_ccsd_21_trans_aiajcj(t2, nocc, nactive, a, i, j, c)
call sigup_nondiag(jac_ibra_iket, ibra, iket)
 ! jac_temp(ibra, iket) = jac_ibra_iket 
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac_ibra_iket = eom_ccsd_21_trans_aibiak(t2, nocc, nactive, a, i, b, k)
call sigup_nondiag(jac_ibra_iket, ibra, iket)
 ! jac_temp(ibra, iket) = jac_ibra_iket 
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac_ibra_iket = eom_ccsd_21_trans_aibjai(t2, nocc, nactive, a, i, b, j)
call sigup_nondiag(jac_ibra_iket, ibra, iket)
 ! jac_temp(ibra, iket) = jac_ibra_iket 
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac_ibra_iket = eom_ccsd_21_trans_aibjaj(t2, nocc, nactive, a, i, b, j)
call sigup_nondiag(jac_ibra_iket, ibra, iket)
 ! jac_temp(ibra, iket) = jac_ibra_iket 
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac_ibra_iket = eom_ccsd_21_trans_aibibk(t2, nocc, nactive, a, i, b, k)
call sigup_nondiag(jac_ibra_iket, ibra, iket)
 ! jac_temp(ibra, iket) = jac_ibra_iket 
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac_ibra_iket = eom_ccsd_21_trans_aibjbi(t2, nocc, nactive, a, i, b, j)
call sigup_nondiag(jac_ibra_iket, ibra, iket)
 ! jac_temp(ibra, iket) = jac_ibra_iket 
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac_ibra_iket = eom_ccsd_21_trans_aibjbj(t2, nocc, nactive, a, i, b, j)
call sigup_nondiag(jac_ibra_iket, ibra, iket)
 ! jac_temp(ibra, iket) = jac_ibra_iket 
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac_ibra_iket = eom_ccsd_21_trans_aibici(t2, nocc, nactive, a, i, b, c)
call sigup_nondiag(jac_ibra_iket, ibra, iket)
 ! jac_temp(ibra, iket) = jac_ibra_iket 
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac_ibra_iket = eom_ccsd_21_trans_aiaiak(t2, nocc, nactive, a, i, k)
call sigup_nondiag(jac_ibra_iket, ibra, iket)
 ! jac_temp(ibra, iket) = jac_ibra_iket 
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac_ibra_iket = eom_ccsd_21_trans_aiajai(t2, nocc, nactive, a, i, j)
call sigup_nondiag(jac_ibra_iket, ibra, iket)
 ! jac_temp(ibra, iket) = jac_ibra_iket 
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac_ibra_iket = eom_ccsd_21_trans_aiajaj(t2, nocc, nactive, a, i, j)
call sigup_nondiag(jac_ibra_iket, ibra, iket)
 ! jac_temp(ibra, iket) = jac_ibra_iket 
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac_ibra_iket = eom_ccsd_21_trans_aiaici(t2, nocc, nactive, a, i, c)
call sigup_nondiag(jac_ibra_iket, ibra, iket)
 ! jac_temp(ibra, iket) = jac_ibra_iket 
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac_ibra_iket = eom_ccsd_21_trans_aibiai(t2, nocc, nactive, a, i, b)
call sigup_nondiag(jac_ibra_iket, ibra, iket)
 ! jac_temp(ibra, iket) = jac_ibra_iket 
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac_ibra_iket = eom_ccsd_21_trans_aibibi(t2, nocc, nactive, a, i, b)
call sigup_nondiag(jac_ibra_iket, ibra, iket)
 ! jac_temp(ibra, iket) = jac_ibra_iket 
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac_ibra_iket = eom_ccsd_21_trans_aiaiai(t2, nocc, nactive, a, i)
call sigup_nondiag(jac_ibra_iket, ibra, iket)
 ! jac_temp(ibra, iket) = jac_ibra_iket 
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac_ibra_iket = eom_ccsd_21_trans_aibjck(t2, nocc, nactive, a, i, b, j, c, k)
call sigup_nondiag(jac_ibra_iket, ibra, iket)
 ! jac_temp(ibra, iket) = jac_ibra_iket 
...
end subroutine ccjac_21_dav
end module ccjac_block_21_dav
