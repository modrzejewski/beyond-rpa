module ccjac_block_21_cc3
use eom_cc3_21_trans
use davidson_main
 
use cc_gparams 
 implicit none
!
! File generated automatically on 2012-05-15 01:40:53 UTC.
!
contains
 
subroutine ccjac_21_cc3(jac, eorb, t1, t2, nocc0, nocc1, nvirt0, nvirt1, bra0, ket0)
...
iket = ketoffset + ck
jac(ibra, iket) = eom_cc3_21_trans_aibjak(eorb, t2, t1, nocc, nactive, a, i, b, j, k)
end do i_aibjak
end do a_aibjak
end do j_aibjak
end do b_aibjak
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac(ibra, iket) = eom_cc3_21_trans_aibjbk(eorb, t2, t1, nocc, nactive, a, i, b, j, k)
end do i_aibjbk
end do a_aibjbk
end do j_aibjbk
end do b_aibjbk
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac(ibra, iket) = eom_cc3_21_trans_aiajck(eorb, t2, t1, nocc, nactive, a, i, j, c, k)
end do i_aiajck
end do a_aiajck
end do j_aiajck
end do k_aiajck
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac(ibra, iket) = eom_cc3_21_trans_aibick(eorb, t2, t1, nocc, nactive, a, i, b, c, k)
end do i_aibick
end do a_aibick
end do b_aibick
end do k_aibick
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac(ibra, iket) = eom_cc3_21_trans_aibjci(eorb, t2, t1, nocc, nactive, a, i, b, j, c)
end do i_aibjci
end do a_aibjci
end do j_aibjci
end do b_aibjci
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac(ibra, iket) = eom_cc3_21_trans_aibjcj(eorb, t2, t1, nocc, nactive, a, i, b, j, c)
end do i_aibjcj
end do a_aibjcj
end do j_aibjcj
end do b_aibjcj
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac(ibra, iket) = eom_cc3_21_trans_aiajak(eorb, t2, t1, nocc, nactive, a, i, j, k)
end do i_aiajak
end do a_aiajak
end do j_aiajak
end do k_aiajak
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac(ibra, iket) = eom_cc3_21_trans_aibiak(eorb, t2, t1, nocc, nactive, a, i, b, k)
end do i_aibiak
end do a_aibiak
end do b_aibiak
end do k_aibiak
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac(ibra, iket) = eom_cc3_21_trans_aibjai(eorb, t2, t1, nocc, nactive, a, i, b, j)
end do i_aibjai
end do a_aibjai
end do j_aibjai
end do b_aibjai
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac(ibra, iket) = eom_cc3_21_trans_aibjaj(eorb, t2, t1, nocc, nactive, a, i, b, j)
end do i_aibjaj
end do a_aibjaj
end do j_aibjaj
end do b_aibjaj
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac(ibra, iket) = eom_cc3_21_trans_aibibk(eorb, t2, t1, nocc, nactive, a, i, b, k)
end do i_aibibk
end do a_aibibk
end do b_aibibk
end do k_aibibk
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac(ibra, iket) = eom_cc3_21_trans_aibjbi(eorb, t2, t1, nocc, nactive, a, i, b, j)
end do i_aibjbi
end do a_aibjbi
end do j_aibjbi
end do b_aibjbi
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac(ibra, iket) = eom_cc3_21_trans_aibjbj(eorb, t2, t1, nocc, nactive, a, i, b, j)
end do i_aibjbj
end do a_aibjbj
end do j_aibjbj
end do b_aibjbj
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac(ibra, iket) = eom_cc3_21_trans_aiaick(eorb, t2, t1, nocc, nactive, a, i, c, k)
end do i_aiaick
end do a_aiaick
end do k_aiaick
end do c_aiaick
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac(ibra, iket) = eom_cc3_21_trans_aiajci(eorb, t2, t1, nocc, nactive, a, i, j, c)
end do i_aiajci
end do a_aiajci
end do j_aiajci
end do c_aiajci
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac(ibra, iket) = eom_cc3_21_trans_aiajcj(eorb, t2, t1, nocc, nactive, a, i, j, c)
end do i_aiajcj
end do a_aiajcj
end do j_aiajcj
end do c_aiajcj
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac(ibra, iket) = eom_cc3_21_trans_aibici(eorb, t2, t1, nocc, nactive, a, i, b, c)
end do i_aibici
end do a_aibici
end do b_aibici
end do c_aibici
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac(ibra, iket) = eom_cc3_21_trans_aiaiak(eorb, t2, t1, nocc, nactive, a, i, k)
end do i_aiaiak
end do a_aiaiak
end do k_aiaiak
!
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac(ibra, iket) = eom_cc3_21_trans_aiajai(eorb, t2, t1, nocc, nactive, a, i, j)
end do i_aiajai
end do a_aiajai
end do j_aiajai
!
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac(ibra, iket) = eom_cc3_21_trans_aiajaj(eorb, t2, t1, nocc, nactive, a, i, j)
end do i_aiajaj
end do a_aiajaj
end do j_aiajaj
!
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac(ibra, iket) = eom_cc3_21_trans_aibiai(eorb, t2, t1, nocc, nactive, a, i, b)
end do i_aibiai
end do a_aibiai
end do b_aibiai
!
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac(ibra, iket) = eom_cc3_21_trans_aibibi(eorb, t2, t1, nocc, nactive, a, i, b)
end do i_aibibi
end do a_aibibi
end do b_aibibi
!
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac(ibra, iket) = eom_cc3_21_trans_aiaici(eorb, t2, t1, nocc, nactive, a, i, c)
end do i_aiaici
end do a_aiaici
end do c_aiaici
!
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac(ibra, iket) = eom_cc3_21_trans_aiaiai(eorb, t2, t1, nocc, nactive, a, i)
end do i_aiaiai
end do a_aiaiai
!
! Elementary loop 25
...
((2 * npair - bj + 2) * (bj - 1)) / 2 + ai - bj + 1
iket = ketoffset + ck
jac(ibra, iket) = eom_cc3_21_trans_aibjck(eorb, t2, t1, nocc, nactive, a, i, b, j, c, k)
end do i_aibjck
end do a_aibjck
end do j_aibjck
end do b_aibjck
...
end subroutine ccjac_21_cc3
end module ccjac_block_21_cc3
