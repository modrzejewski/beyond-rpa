module ccjac_block_12_tripletm_dav
use eom_ccsd_12_tripletm_trans
use davidson_main
use math_constants
use arithmetic
use cmpidx
...
implicit none
!
! File generated automatically on 2014-04-09 17:04:20 UTC.
!
contains
subroutine ccjac_12_tripletm_dav(sigup_diag, sigup_nondiag, t2, nocc0, nocc1, nvirt0, &
...
iket = ketoffset + &
((bj - 1) * (bj - 2)) / 2 + ck
jac_ibra_iket = eom_ccsd_12_tripletm_trans_aibjak(i, b, j, k)
!print*, 'loop z 12 tripletm'
...
iket = ketoffset + &
((bj - 1) * (bj - 2)) / 2 + ck
jac_ibra_iket = eom_ccsd_12_tripletm_trans_aiajck(i, j, c, k)
!print*, 'loop z 12 tripletm'
call sigup_nondiag(jac_ibra_iket, ibra, iket) 
...
iket = ketoffset + &
((bj - 1) * (bj - 2)) / 2 + ck
jac_ibra_iket = eom_ccsd_12_tripletm_trans_aibjci(a, b, j, c)
!print*, 'loop z 12 tripletm'
call sigup_nondiag(jac_ibra_iket, ibra, iket) 
...
iket = ketoffset + &
((bj - 1) * (bj - 2)) / 2 + ck
jac_ibra_iket = eom_ccsd_12_tripletm_trans_aibick(a, b, c, k)
!print*, 'loop z 12 tripletm'
call sigup_nondiag(jac_ibra_iket, ibra, iket) 
...
iket = ketoffset + &
((bj - 1) * (bj - 2)) / 2 + ck
jac_ibra_iket = eom_ccsd_12_tripletm_trans_aiajak(a, i, j, k)
!print*, 'loop z 12 tripletm'
call sigup_nondiag(jac_ibra_iket, ibra, iket) 
...
iket = ketoffset + &
((bj - 1) * (bj - 2)) / 2 + ck
jac_ibra_iket = eom_ccsd_12_tripletm_trans_aibjai(nocc, a, i, b, j)
!print*, 'loop z 12 tripletm'
call sigup_nondiag(jac_ibra_iket, ibra, iket) 
...
iket = ketoffset + &
((bj - 1) * (bj - 2)) / 2 + ck
jac_ibra_iket = eom_ccsd_12_tripletm_trans_aibjaj(i, b, j)
!print*, 'loop z 12 tripletm'
call sigup_nondiag(jac_ibra_iket, ibra, iket) 
...
iket = ketoffset + &
((bj - 1) * (bj - 2)) / 2 + ck
jac_ibra_iket = eom_ccsd_12_tripletm_trans_aibiak(a, i, b, k)
!print*, 'loop z 12 tripletm'
call sigup_nondiag(jac_ibra_iket, ibra, iket) 
...
iket = ketoffset + &
((bj - 1) * (bj - 2)) / 2 + ck
jac_ibra_iket = eom_ccsd_12_tripletm_trans_aibjbi(a, b, j)
!print*, 'loop z 12 tripletm'
call sigup_nondiag(jac_ibra_iket, ibra, iket) 
...
iket = ketoffset + &
((bj - 1) * (bj - 2)) / 2 + ck
jac_ibra_iket = eom_ccsd_12_tripletm_trans_aibibk(a, b, k)
!print*, 'loop z 12 tripletm'
call sigup_nondiag(jac_ibra_iket, ibra, iket) 
...
iket = ketoffset + &
((bj - 1) * (bj - 2)) / 2 + ck
jac_ibra_iket = eom_ccsd_12_tripletm_trans_aiajci(a, i, j, c)
!print*, 'loop z 12 tripletm'
call sigup_nondiag(jac_ibra_iket, ibra, iket) 
...
iket = ketoffset + &
((bj - 1) * (bj - 2)) / 2 + ck
jac_ibra_iket = eom_ccsd_12_tripletm_trans_aiajcj(i, j, c)
!print*, 'loop z 12 tripletm'
call sigup_nondiag(jac_ibra_iket, ibra, iket) 
...
iket = ketoffset + &
((bj - 1) * (bj - 2)) / 2 + ck
jac_ibra_iket = eom_ccsd_12_tripletm_trans_aiaick(nocc, a, i, c, k)
!print*, 'loop z 12 tripletm'
call sigup_nondiag(jac_ibra_iket, ibra, iket) 
...
iket = ketoffset + &
((bj - 1) * (bj - 2)) / 2 + ck
jac_ibra_iket = eom_ccsd_12_tripletm_trans_aibici(a, i, b, c)
!print*, 'loop z 12 tripletm'
call sigup_nondiag(jac_ibra_iket, ibra, iket) 
...
iket = ketoffset + &
((bj - 1) * (bj - 2)) / 2 + ck
jac_ibra_iket = eom_ccsd_12_tripletm_trans_aiajai(nocc, a, i, j)
!print*, 'loop z 12 tripletm'
call sigup_nondiag(jac_ibra_iket, ibra, iket) 
...
iket = ketoffset + &
((bj - 1) * (bj - 2)) / 2 + ck
jac_ibra_iket = eom_ccsd_12_tripletm_trans_aiaiak(nocc, a, i, k)
!print*, 'loop z 12 tripletm'
call sigup_nondiag(jac_ibra_iket, ibra, iket) 
...
iket = ketoffset + &
((bj - 1) * (bj - 2)) / 2 + ck
jac_ibra_iket = eom_ccsd_12_tripletm_trans_aibiai(nocc, a, i, b)
!print*, 'loop z 12 tripletm'
call sigup_nondiag(jac_ibra_iket, ibra, iket) 
...
iket = ketoffset + &
((bj - 1) * (bj - 2)) / 2 + ck
jac_ibra_iket = eom_ccsd_12_tripletm_trans_aiaici(nocc, a, i, c)
!print*, 'loop z 12 tripletm'
call sigup_nondiag(jac_ibra_iket, ibra, iket) 
...
!!print*, '12m', sum
! ! end if
end subroutine ccjac_12_tripletm_dav
end module ccjac_block_12_tripletm_dav
