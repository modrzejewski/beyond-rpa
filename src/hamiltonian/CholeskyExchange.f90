...
contains
      ...
      subroutine chf_RkipRkiq(Kpq, Wkip, Ni, NCholesky, TargetBlockDim, NAO, KScal)
            ...
            do mn = ThisImage, (NBlocks * (NBlocks + 1)) / 2, NImages
                  ...
                  !
                  call real_aTb_x(KpqBlock, Np, Wkip(:, p0:p1), NCholesky*Ni, &
      ...
      subroutine chf_Kpq(Kpq, Rkpq, Cpi, NAO, NCholesky, MaxBufferDim, TargetBlockDim, &
            ...
            if (.not. MaxNik > 0) then
                  call msg("Buffer size too small for Rkpq->Rkiq", MSG_ERROR)
                  error stop
            end if
            NPasses = NI / MaxNik
      ...
      subroutine chf_K(F, C_ao, NOcc, AOBasis, Rkpq, CholeskyBasis, &
            ...
            ! K(p,q) = Sum(i) (pi|iq) = Sum(ki) Rkip*Rkiq
            !
            ! Closed-shell systems:
            ! F(p,q) <- F(p,q) - KScal * K(p,q)
            !
            ! Open-shell systems:
            ! F(p,q,s) <- F(p,q) - KScal * K(p,q,s)
            !
            ! Only the leftmost NOcc columns of C_ao are referenced.
            ! K(p,q) is *not* summed over concurrent images.
            ! 
            !
